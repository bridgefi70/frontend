      ...
      <div className="mx-auto max-w-[1200px]">
        ...
        <div className="space-y-4">
          {events.map((event) => (
            <article
              key={event.title}
              className="grid gap-5 rounded-lg border border-[#29243A] p-4 sm:p-5 md:grid-cols-[150px_1fr] md:gap-5"
            >
              {/* IMAGE */}
              <img
                src={event.image}
                alt={event.title}
                className="h-[180px] w-full rounded-md object-cover md:h-[170px]"
              />
              ...
              <div>
                ...
                <h3 className="mt-4 text-lg font-medium text-white sm:text-xl md:text-2xl">
                  {event.title}
                </h3>

                <p className="mt-2 text-xs leading-5 text-[#858196] sm:text-sm sm:leading-6">
                  {event.description}
                </p>
